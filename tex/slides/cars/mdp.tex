...
\begin{definitionblock}{Policy $\pi: \mathcal{S} \to \mathcal{A}$}
    ...
    A mapping from states $s \in \mathcal{S}$ to actions $a \in \mathcal{A}$. As a function: $a = \pi(s)$
\end{definitionblock}
...
\begin{tikzpicture}[remember picture, overlay]
    ...
    \node[xshift=-5.55cm, yshift=2.7cm, minimum size=0.1cm, inner sep=0pt, outer sep=0pt, draw=black, fill=black, circle] (action-base) at (current page.south east) {};
    \node[xshift=-2.5cm, yshift=2.3cm, text width=3cm, align=center] (action-label) at (current page.south east) {Take action \texttt{down} in state $(9,4)$. So $\pi\bigl((9,4)\bigr) = \texttt{down}$};
    \draw[->, dashed] (action-base) -- (action-label);
\end{tikzpicture}
